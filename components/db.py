...
# ══════════════════════════════════════════════════════════════
# LEAD TIME — ORÇAMENTO → REQUISIÇÃO → NF
# Tabela Supabase: genius_leadtime_pecas
# Colunas:
#   id, Nr_Orcamento, Cliente_Revenda, Valor_Total,
#   Data_Orcamento_Fechado,   (date str DD/MM/YYYY)
#   Nr_Requisicao,            (string)
#   Data_Requisicao,          (date str DD/MM/YYYY)
#   Nr_NF,                    (string, preenchido ao emitir NF)
#   Data_NF,                  (date str DD/MM/YYYY, preenchido ao emitir NF)
...
def adicionar_leadtime(reg: dict) -> bool:
    ...
    if not sb: return False
    try:
        payload = {k: str(reg.get(k, "")) for k in _COLS_LEAD}
        res = _exec_safe(sb.table("genius_leadtime_pecas").insert(payload), write=True)
        return res is not None
    except Exception as e:
        st.error(f"❌ Erro ao salvar lead time: {e}"); return False
...
def atualizar_leadtime(row_id: int, dados: dict) -> bool:
    ...
    try:
        ...
        res = _exec_safe(sb.table("genius_leadtime_pecas").update(payload).eq("id", row_id), write=True)
        return res is not None
    except Exception as e:
        st.error(f"❌ Erro ao atualizar lead time: {e}"); return False
...
def calcular_kpis_leadtime(df: pd.DataFrame) -> dict:
    ...
    Lead time = dias entre Data_Orcamento_Fechado e Data_NF (quando NF emitida).
    """
    _zero = {"total_registros": 0, "aguardando_req": 0,
             "req_enviada": 0, "nf_emitida": 0,
             "lead_medio_dias": None, "lead_max_dias": None}
    if df is None or df.empty:
        return _zero

    total        = len(df)
    ag_req       = int((df["Status_Lead"] == "Orçamento Fechado").sum())
    req_env      = int((df["Status_Lead"] == "Req. Enviada").sum())
    nf_emit      = int((df["Status_Lead"] == "NF Emitida").sum())

    # Calcula lead time apenas dos registros com NF emitida
    lead_dias = []
    for _, row in df[df["Status_Lead"] == "NF Emitida"].iterrows():
        try:
            from datetime import datetime
            d_oc = datetime.strptime(str(row["Data_Orcamento_Fechado"]).strip(), "%d/%m/%Y")
            d_nf = datetime.strptime(str(row["Data_NF"]).strip(), "%d/%m/%Y")
            delta = (d_nf - d_oc).days
            if delta >= 0:
                lead_dias.append(delta)
        except Exception:
            pass

    return {
        "total_registros": total,
        "aguardando_req":  ag_req,
        "req_enviada":     req_env,
        "nf_emitida":      nf_emit,
        "lead_medio_dias": round(sum(lead_dias) / len(lead_dias), 1) if lead_dias else None,
        "lead_max_dias":   max(lead_dias) if lead_dias else None,
    }
